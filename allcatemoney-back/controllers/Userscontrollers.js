  ...
  register: async (req, res) => {
    ...
    try {
      ...
      await pool.query(
        ...
        [userid, "register", "User registered via API"]
      );
  ...
  /*   console.log(isChecked)
    ...
    try {
      if (!username || !password) {
        return res
          .status(400)
          .json({ field: "general", message: "กรุณากรอกข้อมูลให้ครบถ้วน" });
      }
      ...
      if (userresult.rowCount === 0) {
        return res
          .status(401)
          .json({ field: "username", message: "อีเมลล์ไม่ถูกต้อง" });
      }
      ...
      if (!ismatch) {
        return res
          .status(401)
          .json({ field: "password", message: "รหัสผ่านไม่ถูกต้อง" });
      }
